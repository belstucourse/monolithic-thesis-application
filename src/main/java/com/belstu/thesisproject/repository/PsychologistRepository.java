...
public interface PsychologistRepository extends JpaRepository<Psychologist, String> {
    Page<Psychologist> findDistinctByTagsInAndVerified(List<Tag> tags, boolean verified, Pageable pageable);
    ...
    @Query(value = "SELECT DISTINCT ON (p.id) * FROM Psychologist p " +
            ...
            "WHERE p.is_verified = true AND pt.tag_id IN ?1 AND (?2 BETWEEN pw.start_date_time AND pw.end_date_time OR ?3 BETWEEN pw.start_date_time AND pw.end_date_time)", nativeQuery = true)
